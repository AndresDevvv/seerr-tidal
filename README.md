...
## Local Dev For This Fork

This fork replaces the old music flow with a bundled `OrpheusDL + TIDAL` setup.

### Start the app locally

Use the local config and local Next build directory so you do not hit permission issues from existing root-owned files:

```bash
cd seerr-lidarr
pnpm install
python3 -m venv vendor/orpheusdl-core/.venv
vendor/orpheusdl-core/.venv/bin/pip install -r vendor/orpheusdl-core/requirements.txt
CONFIG_DIRECTORY=$(pwd)/.config-local NEXT_DIST_DIR=.next-local pnpm dev
```

Open:

```text
http://localhost:5055/setup
```

### Where OrpheusDL lives

Bundled OrpheusDL:

```text
vendor/orpheusdl-core
```

Bundled TIDAL module:

```text
vendor/orpheusdl-core/modules/tidal
```

### TIDAL login

The first time you use music downloads, you must initialize TIDAL auth manually because Seerr runs OrpheusDL non-interactively.

Run this once:

```bash
cd vendor/orpheusdl-core
.venv/bin/python orpheus.py luckysearch tidal album "test"
```

Then:

1. Choose a login method when prompted.
2. Complete the TIDAL auth flow.
3. Let OrpheusDL save the session.

Saved auth ends up in:

```text
vendor/orpheusdl-core/config/loginstorage.bin
```

That file is gitignored in this fork.

### TIDAL module config

The TIDAL module settings live here:

```text
vendor/orpheusdl-core/config/settings.json
```

That file is also gitignored in this fork.

### How to tell if music downloaded

This integration currently runs OrpheusDL synchronously. If Seerr logs:

```text
Sent request to OrpheusDL (TIDAL)
```

the Orpheus command already exited successfully.
