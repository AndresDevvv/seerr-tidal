...
import { buildOrpheusServiceDetails } from '@server/lib/orpheus';
...
requestRoutes.get<Record<string, unknown>, RequestResultsResponse>(
  ...
  async (req, res, next) => {
    try {
      ...
      const lidarrServers = settings.lidarr.map((lidarrSetting) => ({
        id: lidarrSetting.id,
        profiles: buildOrpheusServiceDetails(lidarrSetting).profiles,
      }));
